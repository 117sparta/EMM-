<template>
    <div id="frame">
      <header>
          ...
          <button id="newFolderBtn" @click="newFolderInput">新建文件夹</button>
        ...
        <div>
          <div id="fileType">全部文件</div><div id="loadingState">加载完毕(共{{fileList.length}}个)</div>
        </div>
      </header>
      <!--新建文件夹的组件开始-->
        <div id="newFolderInputSheet">
            <div id="newFolderInput">
              <div id="inputFrame">
                <label id="label1">文件夹名：</label><input type="text" /><br/>
              </div>
              <button class="confirmBtn" @click="fileNameConfirm">确定</button>
              <button class="abortBtn" @click="fileNameInputAbort">取消</button>
            </div>
        </div>
      <!--新建文件夹的组件结束-->
      <delete-warning
        :is-active="deleteWarningIsActive" @deleteConfirm="deleteConfirm"
        @deleteAbort="deleteWarningIsActive=false"
      ></delete-warning>
      <!--重命名组件-->
...
export default {
  name: 'allFile',
  components: {file, deleteWarning, rename},
  data: function () {
    return {
      isSelectedAll: false,
      totalSelectedNum: 0,
 ...
 created: function () {
  ...
  methods: {
    ...
    newFolderInput: function () {
      $('#newFolderInputSheet').show()
    },
    fileNameConfirm: function () {
      $('#newFolderInputSheet').hide()
    },
    fileNameInputAbort: function () {
      $('#newFolderInputSheet').hide()
    },
...
div#frame{
  ...
  .BtnStyle,#uploadBtn,#searchBtn,#newFolderBtn{
    height:1.5em;
    font-size:1em;
    cursor: pointer;
    outline:none;
  ...
  .BtnStyle:hover,#newFolderBtn:hover{
...
.renameBtnDisabled{
  ...
  .confirmBtn:hover,.abortBtn:hover{
    color:rgba(255,255,255,0.7);
  }
  #label1{
    color: #666168;
